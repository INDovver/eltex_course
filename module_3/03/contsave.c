...
int main (){
    int fd = open("contact_file.txt",O_RDWR|O_CREAT);
    if (fd == -1) {
    ...
    clearCons();
    Person contBook[MC]={};
    ssize_t size = read(fd,contBook,sizeof(contBook));
    if (size ==-1){
        perror("read error");
        exit(EXIT_FAILURE);
    }
    int contactNum = size/sizeof(Person);
        ...
        if (inp[0]>='0'&& inp[0]<='9'){
            ...
            switch (inp[0]){
                ...
                case 'q':
                    ...
                    lseek(fd, 0, SEEK_SET);
                    if(write(fd, contBook, sizeof(Person)*contactNum)==-1){
                        perror("write error");
                        exit(EXIT_FAILURE);
                    };
                    if (close(fd) == -1) {
